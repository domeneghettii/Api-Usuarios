...
const router = {
    ...
    addPost: (req, res) => {
        try {
            const {title, description, url, like, coments} = req.body;
            if (!title || !description || !url || like, coments === undefined) {
                throw new Error("preencha todos os campos!!");
            }
            const newPost = new Post(title, description, url, like, coments);
